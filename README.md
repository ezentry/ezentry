# Hi, I'm Alex M👋

**Web game developer** — I build HTML5 games and interactive web experiences with JS/TS, Phaser, PixiJS, and Three.js.

## What I do

- 🎮 **HTML5 games** — browser games shipped and live on web game portals
- 🕹️ **Playable ads** — interactive HTML5 creatives for mobile user acquisition
- 🧩 **Telegram Mini Apps** — games and mini-apps with TMA SDK, Stars payments, and viral hooks
- 🎨 **3D web experiences** — Three.js scenes and widgets for landing pages and product showcases

## Tech Stack

![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Three.js](https://img.shields.io/badge/-Three.js-000000?style=flat&logo=three.js&logoColor=white)
![Phaser](https://img.shields.io/badge/-Phaser-8A2BE2?style=flat&logo=phaser&logoColor=white)
![PixiJS](https://img.shields.io/badge/-PixiJS-E91E63?style=flat&logo=pixijs&logoColor=white)
![Vite](https://img.shields.io/badge/-Vite-646CFF?style=flat&logo=vite&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat&logo=node.js&logoColor=white)

## AI-Assisted Development

I build with AI coding assistants as part of my workflow:

![Claude Code](https://img.shields.io/badge/-Claude%20Code-D97757?style=flat&logo=anthropic&logoColor=white)
![Cursor](https://img.shields.io/badge/-Cursor-000000?style=flat&logo=cursor&logoColor=white)
![GitHub Copilot](https://img.shields.io/badge/-GitHub%20Copilot-181717?style=flat&logo=github&logoColor=white)
![ChatGPT](https://img.shields.io/badge/-ChatGPT-74AA9C?style=flat&logo=openai&logoColor=white)

AI helps me move fast, but every project I ship is reviewed, tested, and maintained by me — architecture, performance, and platform specs are my responsibility.

## Selected Work

- 🕹️ [Playable Ad demo](#) — 2.4 MB single-file HTML5 build
- 📱 [Telegram Mini App demo](#) — TMA SDK + Stars + referrals
- 🧊 [3D widget demo](#) — Three.js, 60 FPS on mobile

## Contact

- 🌐 Website: [ainy.space](#)
- 🐦 X: [@ainyspace](#)
- 💬 Telegram: [ezgamez](#)


---

*Open to freelance and B2B work: HTML5 games, playable ads, Telegram Mini Apps, and 3D web.*
